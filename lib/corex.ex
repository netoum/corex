defmodule Corex do
    # https://github.com/corexlabs/corexcss/releases
    @latest_version "1.0.0"

    @moduledoc """
    Tailwind is an installer and runner for [corex](https://corexcss.com/).

    ## Profiles

    You can define multiple corex profiles. By default, there is a
    ...
    There are two global configurations for the corex application:

      * `:version` - the expected corex version

      * `:version_check` - whether to perform the version check or not.
        Useful when you manage the corex executable with an external
        tool (eg. npm)

      * `:cacerts_path` - the directory to find certificates for
        https connections

      * `:path` - the path to find the corex executable at. By
        default, it is automatically downloaded and placed inside
        the `_build` directory of your current app

    Overriding the `:path` is not recommended, as we will automatically
    download and manage `corex` for you. But in case you can't download
    it (for example, GitHub behind a proxy), you may want to
    set the `:path` to a configurable system location.

    For instance, you can install `corex` globally with `npm`:

        $ npm install -g corexcss

    On Unix, the executable will be at:

        NPM_ROOT/corex/node_modules/corex-TARGET/bin/corex

    On Windows, it will be at:

        NPM_ROOT/corex/node_modules/corex-windows-(32|64)/corex.exe
    ...
    your configuration file:
        ...
        config :corex, path: System.get_env("MIX_TAILWIND_PATH")
    ...
    @doc false
    def start(_, _) do
      if Application.get_env(:corex, :version_check, true) do
        unless Application.get_env(:corex, :version) do
          Logger.warning("""
          corex version is not configured. Please set it in your config files:

              config :corex, :version, "#{latest_version()}"
          """)
        end
        ...
        case bin_version() do
          ...
          {:ok, version} ->
            Logger.warning("""
            Outdated corex version. Expected #{configured_version}, got #{version}. \
            Please run `mix corex.install` or update the version in your config files.\
            """)
    ...
    @doc """
    Returns the configured corex version.
    """
    def configured_version do
      Application.get_env(:corex, :version, latest_version())
    end
    ...
    def config_for!(profile) when is_atom(profile) do
      Application.get_env(:corex, profile) ||
        raise ArgumentError, """
        unknown corex profile. Make sure the profile is defined in your config/config.exs file, such as:
    ...
    def bin_path do
      ...
      Application.get_env(:corex, :path) ||
    ...
    @doc """
    Returns the version of the corex executable.
    ...
    @doc """
    Installs, if not available, and then runs `corex`.
    ...
    def default_base_url do
      "https://github.com/netoum/corex_js/releases/download/$version/corex-$target"
    end

    @doc """
    Installs corex with `configured_version/0`.
    """
    ...
    # Available targets:
    #  corexcss-freebsd-arm64
    #  corexcss-freebsd-x64
    #  corexcss-linux-arm64
    #  corexcss-linux-x64
    #  corexcss-linux-armv7
    #  corexcss-macos-arm64
    #  corexcss-macos-x64
    #  corexcss-windows-x64.exe
    defp target do
      ...
      case {:os.type(), arch, :erlang.system_info(:wordsize) * 8} do
        ...
        {_os, _arch, _wordsize} -> raise "corex is not available for architecture: #{arch_str}"
      end
    end
    ...
    defp fetch_body!(url, retry \\ true) do
      ...
      Logger.debug("Downloading corex from #{url}")
        ...
        other ->
          ...
          You can try again later and, if that does not work, you might:
            ...
            2. Manually download the executable from the URL above and
               place it inside "_build/corex-#{target()}"

            3. Install and use Tailwind from npmJS. See our module documentation
               to learn more: https://hexdocs.pm/corex
          """
      end
    end
    ...
    defp cacertfile() do
      Application.get_env(:corex, :cacerts_path) || CAStore.file_path()
    end
